#!/usr/bin/env bash
#
# Create a base CentOS docker image for GlusterFS
#
...
prepare ()
{
    ...
    yum -c "$yum_config" --installroot="$target" --setopt=tsflags=nodocs \
        --setopt=group_package_types=mandatory -y install glusterfs-server glusterfs-fuse glusterfs nfs-utils rpcbind
